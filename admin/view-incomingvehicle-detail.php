...
if(isset($_POST['submit']))
  {
    ...
    $query1 = mysqli_query($con, "select ParkingNumber from tblvehicle where ID='$cid'");
    ...
    if ($query1) {
      ...
      if ($row) {
          ...
          echo "The parking number is: $parkingNumber";
      } else {
          echo "No matching record found for ID $cid";
      }
  } else {
    ...
    //$query1 = mysqli_fetch_array($con, "select ParkingNumber from tblvehicle where ID='$cid'");
    //$query=mysqli_query($con, "update  tblvehicle set Remark='$remark',Status='$status',ParkingCharge='$parkingcharge' where ID='$cid'");
    //$query=mysqli_query($con, "update  tblvehicle set Remark='$remark',Status='$status',ParkingCharge='$parkingcharge' where ID='$cid'");
    if ($parkingNumber==$parkingcharge) {
     $query=mysqli_query($con, "update  tblvehicle set Remark='$remark',Status='$status',ParkingCharge='$parkingcharge' where ID='$cid'");  
     echo "<script>alert('Parking Number Valid');</script>";
  }
  else
    {
    echo "<script>alert('Invalid Parking Number. Please try again');</script>";
    }
